...
#
# EMCP - added to support custom datasets with alpha channels
#
__C.PIXEL_MEANS_ALPHA_CHANNEL = np.array([[[102.9801, 115.9465, 122.7717, 0]]])
...
def get_output_dir(imdb, weights_filename):
    ...
    (if not None).
    """
    outdir = os.path.abspath(os.path.join(__C.DATA_DIR, "..", 'output', __C.EXP_DIR, imdb.name))
...
def get_output_tb_dir(imdb, weights_filename):
    ...
    (if not None).
    """
    outdir = os.path.abspath(os.path.join(__C.ROOT_DIR, 'tensorboard', __C.EXP_DIR, imdb.name))
    if weights_filename is None:
        weights_filename = 'default'
    outdir = os.path.join(outdir, weights_filename)
    if not os.path.exists(outdir):
